...
let s:cleverf_find_line=-1|let s:cleverf_find_last=0
func! s:CleverFDeleteHl() abort
	if s:cleverf_find_now==getline(line("."))[col(".")-1]&&s:cleverf_find_line==line('.')|return|endif
...
func! s:CleverF(ch) abort
	let line=getline('.')|let line_num=line('.')|let col=col('.')
	if type(s:cleverf_find_now)==0&&a:ch==#'F'|call feedkeys('F','n')|return|endif
	if type(s:cleverf_find_now)!=0
		let i=0
		if a:ch!=#'F'|while i<len(s:cleverf_pos_arr)&&col>s:cleverf_pos_arr[i]|let i=i+1|endwhile
		else|let i=len(s:cleverf_pos_arr)-1|while i>0&&col-1<=s:cleverf_pos_arr[i]|let i=i-1|endwhile
		endif
		if i>=0&&i<len(s:cleverf_pos_arr)|call cursor(line('.'),s:cleverf_pos_arr[i]+1)|endif
		return
	endif
	if a:ch==#'f'
	...
	let s:cleverf_pos_arr=[]|let pos=stridx(line,ch,col)
	...
	for now_pos in s:cleverf_pos_arr
		let tmp = matchaddpos("incsearch", [[line_num,now_pos+1,1]])|call add(s:cleverf_hl_arr,tmp)
	endfor
	call cursor(line('.'), s:cleverf_pos_arr[0]+1)|let s:cleverf_find_now=ch|let s:cleverf_find_line=line_num
